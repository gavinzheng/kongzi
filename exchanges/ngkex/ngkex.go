...
const (
	...
	ngkexAPIContext	= "openapi"
	ngkexAPIVersion = "1"

	ngkexMarketHistoryKline    = "klines"							// NGKEX
	//ngkexMarketDetail          = "market/detail"
	ngkexMarketDetailMerged    = "depth/merged"  					// NGKEX
	ngkexMarketDepth           = "depth" 							// NGKEX
	ngkexMarketTrade           = "trades"							// NGKEX
	//ngkexMarketTradeHistory    = "market/history/trade"
	ngkexSymbols               = "pairs"							// NGKEX
	//ngkexCurrencies            = "common/currencys"
	ngkexTimestamp             = "time"								// NGKEX
	ngkexAccounts              = "subAccount/query"					// NGKEX
	ngkexAccountBalance        = "account"							// NGKEX
	//ngkexAggregatedBalance     = "subuser/aggregate-balance"
	ngkexOrderPlace            = "order"							// NGKEX
	ngkexOrderCancel           = "order/%s"							// NGKEX
	//ngkexOrderCancelBatch      = "order/orders/batchcancel"
	//ngkexBatchCancelOpenOrders = "order/orders/batchCancelOpenOrders"
	ngkexGetOrder              = "order/%s"							// NGKEX
	//ngkexGetOrderMatch         = "order/orders/%s/matchresults"
	ngkexGetOrders             = "order/orders"
	ngkexGetOpenOrders         = "order/openOrders"					// NGKEX
	...
	ngkexWithdrawCreate        = "withdraw"							// NGKEX
...
func (h *NGKEX) GetSpotKline(arg KlinesRequestParams) ([][]interface{}, error) {
	vals := url.Values{}
	vals.Set("symbol", arg.Symbol)
	vals.Set("interval", string(arg.Period))
	...
	type response struct {
		Response
		//Data1 []KlineItem // `json:"data"`
		Data [][]interface{}
	}

	var result response
	urlPath := fmt.Sprintf("%s/quote/v%s/%s", h.APIUrl, ngkexAPIVersion, ngkexMarketHistoryKline)

	err := h.SendHTTPRequest(common.EncodeURLValues(urlPath, vals), &result.Data)
...
func (h *NGKEX) GetMarketDetailMerged(symbol string) (DetailMerged, error) {
	...
	var result response
	urlPath := fmt.Sprintf("%s/quote/v%s/%s", h.APIUrl, ngkexAPIVersion, ngkexMarketDetailMerged)

	err := h.SendHTTPRequest(common.EncodeURLValues(urlPath, vals), &result.Tick)
	if result.ErrorMessage != "" {
...
func (h *NGKEX) GetDepth(obd OrderBookDataRequestParams) (Orderbook, error) {
	...
	var result response
	urlPath := fmt.Sprintf("%s/quote/v%s/%s", h.APIUrl, ngkexAPIVersion, ngkexMarketDepth)
...
func (h *NGKEX) GetTrades(symbol string) ([]Trade, error) {
	...
	var result response
	urlPath := fmt.Sprintf("%s/quote/v%s/%s", h.APIUrl, ngkexAPIVersion, ngkexMarketTrade)
...
// GetCurrencies returns a list of currencies supported by Ngkex
...
// GetTimestamp returns the Ngkex server time
...
// GetAccounts returns the Ngkex user accounts
func (h *NGKEX) GetAccounts() ([]Account, error) {
	...
	var result response
	err := h.SendAuthenticatedHTTPRequest(http.MethodPost, ngkexAccounts, url.Values{}, nil, &result.AccountData)
...
// GetAccountBalance returns the users Ngkex account balance
func (h *NGKEX) GetAccountBalance(accountID string) ([]AccountBalanceDetail, error) {
	type response struct {
		Response
		AccountBalanceData []AccountBalanceDetail `json:"balances"`
	}

	var result response
	endpoint := ngkexAccountBalance
	...
	return result.AccountBalanceData, err
}
...
// SpotNewOrder submits an order to Ngkex
...
// CancelExistingOrder cancels an order on Ngkex
...
func (h *NGKEX) SendAuthenticatedHTTPRequest(method, endpoint string, values url.Values, data, result interface{}) error {
	...
	values.Set("timestamp", fmt.Sprintf("%d", time.Now().Unix())) // .Now().UTC().Format("2006-01-02T15:04:05"))

	endpoint = fmt.Sprintf("/openapi/v%s/%s", ngkexAPIVersion, endpoint)
	//payload := fmt.Sprintf("%s\napi.ngkex.io\n%s\n%s",
	//	method, endpoint, values.Encode())
	payload := fmt.Sprintf("%s", values.Encode())
	...
	headers["X-BH-APIKEY"] = h.APIKey

	hmac := common.GetHMAC(common.HashSHA256, []byte(payload), []byte(h.APISecret))
	signature := hex.EncodeToString(hmac) // common.Base64Encode(hmac)
//	signature =hexutils.BytesToHex(hmac)

...
func (h *NGKEX) wsDial(dialer *websocket.Dialer) error {
...
func (h *NGKEX) wsAuthenticatedDial(dialer *websocket.Dialer) error {
...
func (h *NGKEX) wsMultiConnectionFunnel(ws *wshandler.WebsocketConnection, url string) {
...
func (h *NGKEX) wsHandleAuthenticatedData(resp WsMessage) {
...
func (h *NGKEX) wsHandleMarketData(resp WsMessage) {
	var init WsResponse
	err := common.JSONDecode(resp.Raw, &init)
	if err != nil {
		h.Websocket.DataHandler <- err
...
func (h *NGKEX) WsProcessOrderbook(ob *WsDepth, symbol string) error {
...
func (h *NGKEX) GenerateDefaultSubscriptions() {
...
func (h *NGKEX) Subscribe(channelToSubscribe wshandler.WebsocketChannelSubscription) error {
...
func (h *NGKEX) Unsubscribe(channelToSubscribe wshandler.WebsocketChannelSubscription) error {
	if common.StringContains(channelToSubscribe.Channel, "orders.") ||
		common.StringContains(channelToSubscribe.Channel, "accounts") {
...
func (h *NGKEX) wsGenerateSignature(timestamp, endpoint string) []byte {
	values := url.Values{}
...
func (h *NGKEX) wsAuthenticatedSubscribe(operation, endpoint, topic string) error {
	timestamp := time.Now().UTC().Format(wsDateTimeFormatting)
...
func (h *NGKEX) wsGetAccountsList(pair currency.Pair) (*WsAuthenticatedAccountsListResponse, error) {
	if !h.Websocket.CanUseAuthenticatedEndpoints() {
...
func (h *NGKEX) wsGetOrdersList(accountID int64, pair currency.Pair) (*WsAuthenticatedOrdersResponse, error) {
	if !h.Websocket.CanUseAuthenticatedEndpoints() {
...
func (h *NGKEX) wsGetOrderDetails(orderID string) (*WsAuthenticatedOrderDetailResponse, error) {
	if !h.Websocket.CanUseAuthenticatedEndpoints() {
